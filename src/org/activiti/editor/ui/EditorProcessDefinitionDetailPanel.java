...
public class EditorProcessDefinitionDetailPanel extends DetailPanel {
	...
	protected static final Logger LOGGER = LoggerFactory
			.getLogger(EditorProcessDefinitionDetailPanel.class);
	...
	protected transient RepositoryService repositoryService = ProcessEngines
			.getDefaultProcessEngine().getRepositoryService();

	public EditorProcessDefinitionDetailPanel(String modelId,
	...
	protected void initActions() {
		newModelButton = new Button(
				i18nManager.getMessage(Messages.PROCESS_NEW));
		newModelButton.addListener(new NewModelClickListener());

		importModelButton = new Button(
				i18nManager.getMessage(Messages.PROCESS_IMPORT));
		importModelButton.addListener(new ImportModelClickListener());

		editModelButton = new Button(
				i18nManager.getMessage(Messages.PROCESS_EDIT));
		...
		actionSelect.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if (i18nManager.getMessage(Messages.PROCESS_COPY).equals(
						event.getProperty().getValue())) {
					ExplorerApp
							.get()
							.getViewManager()
							.showPopupWindow(
									new CopyModelPopupWindow(modelData));
				} else if (i18nManager.getMessage(Messages.PROCESS_DELETE)
						.equals(event.getProperty().getValue())) {
					ExplorerApp
							.get()
							.getViewManager()
							.showPopupWindow(
									new DeleteModelPopupWindow(modelData));
				} else if (i18nManager.getMessage(Messages.PROCESS_DEPLOY)
						.equals(event.getProperty().getValue())) {
					deployModel();
				} else if (i18nManager.getMessage(Messages.PROCESS_EXPORT)
						.equals(event.getProperty().getValue())) {
					exportModel();
				}
			}
		...
		processDefinitionPage.getToolBar().setComponentAlignment(actionLabel,
				Alignment.MIDDLE_LEFT);
		processDefinitionPage.getToolBar().addAdditionalComponent(actionSelect);
		processDefinitionPage.getToolBar().setComponentAlignment(actionSelect,
				Alignment.MIDDLE_RIGHT);
	}

	public void initProcessDefinitionInfo() {
		if (definitionInfoComponent == null) {
			definitionInfoComponent = new EditorProcessDefinitionInfoComponent(
					modelData);
		}
	...
	protected void initHeader() {
		...
		// Version
		String versionString = i18nManager.getMessage(Messages.PROCESS_VERSION,
		...
		final FileResource stream = new FileResource(new File(""),
			...
			public DownloadStream getStream() {
				...
				try {
					...
					if (SimpleTableEditorConstants.TABLE_EDITOR_CATEGORY
							.equals(modelData.getCategory())) {
						JsonConverter jsonConverter = new JsonConverter();
						WorkflowDefinition workflowDefinition = jsonConverter
								.convertFromJson(editorNode);
						filename = workflowDefinition.getName();
						WorkflowDefinitionConversion conversion = ExplorerApp
								.get()
						...
						BpmnModel model = jsonConverter
								.convertToBpmnModel(editorNode);
						filename = model.getMainProcess().getId()
								+ ".bpmn20.xml";
					...
					ExplorerApp
							...
							.showErrorNotification(
									Messages.PROCESS_TOXML_FAILED, e);
				}
				return ds;
			}
		};
	...
	protected void deploySimpleTableEditorModel(final ObjectNode modelNode) {
		...
		final DeployModelPopupWindow deployModelPopupWindow = new DeployModelPopupWindow(
				modelData);

		deployModelPopupWindow.getDeployButton().addListener(
				new ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						JsonConverter jsonConverter = new JsonConverter();
						WorkflowDefinition workflowDefinition = jsonConverter
								.convertFromJson(modelNode);

						// Update model name
						modelData.setName(deployModelPopupWindow
								.getProcessName());
						workflowDefinition.setName(deployModelPopupWindow
								.getProcessName());
						...
						byte[] bpmnBytes = null;
						try {
							bpmnBytes = conversion.getbpm20Xml().getBytes(
									"utf-8");
							...
							if (deployModelPopupWindow.isGenerateReports()) {
								ProcessDefinition processDefinition = repositoryService
										...
										.singleResult();
								ReportingUtil
										.generateTaskDurationReport(processDefinition
												.getId());
							}
